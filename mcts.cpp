...
bool MCTS::Update(int action, int observation, double reward)
{
   ...
   // cout << "root part: " << Root->Beliefs().GetNumSamples() << endl;
    if (vnode)
    {
        if (Params.Verbose >= 1)
    ...
    /*int sum = 0;
    for (int j = 0; j < beliefs.GetNumSamples() ; j++)
    {
       sum += (safe_cast<const CAUSAL_FURNITURE_STATE*>(beliefs.GetSample(j)))->numleft ;
    }*/
    //cout << "belief update" << sum << " " << beliefs.GetNumSamples() << endl;
...
void MCTS::AddTransforms(VNODE* root, BELIEF_STATE& beliefs)
{
    ...
    //cout << "numtrans " << Params.NumTransforms << "maxattempt: " << Params.MaxAttempts << endl;
